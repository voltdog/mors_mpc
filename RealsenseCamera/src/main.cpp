...
string GetRequiredEnv(const char* name)
{
    ...
    {
        throw std::runtime_error(string("[RealsenseCamera]: ") + name + " must be set.");
    }
    return value;
}
...
int main(int argc, char * argv[]) try
{
    ...
    // Get config address
    string config_dir = mors_sys::GetEnv("CONFIGPATH");
    string channels_config_address = config_dir + "/channels.yaml";
    string camera_config_address = config_dir + "/realsense_camera.yaml";
    string control_lcm_url = GetRequiredEnv("LCM_CONTROL_URL");

    // Read config
    YAML::Node channel_config = YAML::LoadFile(channels_config_address);
    string odometry_channel = channel_config["odometry"].as<string>();

    std::string configured_serial;
    try
    {
        YAML::Node camera_config = YAML::LoadFile(camera_config_address);
        if (camera_config["stream"] && camera_config["stream"]["serial"])
        {
            configured_serial = camera_config["stream"]["serial"].as<string>();
        }
    }
    catch (const YAML::Exception& e)
    {
        std::cerr << "[RealsenseCamera]: failed to read " << camera_config_address
                  << ": " << e.what() << std::endl;
    }

    // LCM init
    lcm::LCM odometry_publisher(control_lcm_url);
    if (!odometry_publisher.good())
        return EXIT_FAILURE;
    cout << "[RealsenseCamera]: control LCM URL: " << control_lcm_url << endl;
    mors_msgs::odometry_msg odometryMsg;

    // T265 init. Prefer an explicit serial to avoid probing another RealSense
    // device that may already be opened by the D435i module or realsense-viewer.
    std::string serial = configured_serial;
    if (serial.empty())
    {
        if (!device_with_streams({ RS2_STREAM_POSE}, serial))
            return EXIT_SUCCESS;
    }
    std::cout << "[RealsenseCamera]: using T265 serial=" << serial << std::endl;
