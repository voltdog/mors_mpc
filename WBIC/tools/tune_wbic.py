...
LCM_MSG_ROOT = Path("/home/yoggi/lcm_msgs")
...
SWING_PHASES = {0, 2}
STANCE_PHASES = {1, 3}
ScheduleFn = Callable[[float], dict[str, Any]]
...
class ControllerRunner:
    def __init__(self, config_path: Path, lcm_url: str, log_dir: Path):
        ...
        self.processes = [
            ...
            subprocess.Popen(
                [str(REPO_ROOT / "WBIC/build/wbic_ctrl")],
                env=env,
                stdout=(log_dir / "wbic.log").open("w"),
                stderr=subprocess.STDOUT,
            ),
        ]
...
def short_schedule(t: float) -> dict[str, Any]:
    if t < 0.9:
        return warmup_command()
    if t < 1.8:
        alpha = smooth_step(t - 0.9, 0.45)
        return make_command(
            enabled=True,
            standing=True,
            pos=[0.0, 0.0, 0.206],
            orientation=[0.05 * alpha, -0.04 * alpha, 0.05 * alpha],
            lin_vel=[0.0, 0.0, 0.0],
            ang_vel=[0.0, 0.0, 0.0],
            stride_height=0.06,
            t_sw=0.2,
    ...
    if t < 3.0:
        alpha = smooth_step(t - 1.8, 0.6)
        return make_command(
            enabled=True,
            standing=False,
            pos=[0.0, 0.0, 0.2],
            orientation=[0.0, 0.0, 0.0],
            lin_vel=[0.26 * alpha, 0.0, 0.0],
            ang_vel=[0.0, 0.0, 0.0],
            stride_height=0.08,
            t_sw=0.21,
            t_st=0.3,
    ...
    if t < 4.1:
        alpha = smooth_step(t - 3.0, 0.5)
        return make_command(
            enabled=True,
            standing=False,
            pos=[0.0, 0.0, 0.2],
            orientation=[0.0, 0.0, 0.0],
            lin_vel=[0.18, 0.09 * alpha, 0.0],
            ang_vel=[0.0, 0.0, 0.15 * alpha],
            stride_height=0.085,
            t_sw=0.21,
            t_st=0.3,
        )
    return make_command(
        ...
        pos=[0.0, 0.0, 0.2],
        orientation=[0.0, 0.0, 0.0],
        lin_vel=[0.10, 0.06, 0.0],
        ang_vel=[0.0, 0.0, 0.18],
        stride_height=0.085,
        t_sw=0.21,
        t_st=0.3,
    )
...
def build_validation_schedule(lateral_sign: float, yaw_sign: float) -> ScheduleFn:
    def schedule(t: float) -> dict[str, Any]:
        if t < 1.0:
            return warmup_command()
        if t < 2.1:
            alpha = smooth_step(t - 1.0, 0.55)
            return make_command(
                enabled=True,
                standing=True,
                pos=[0.0, 0.0, 0.208],
                orientation=[0.06 * alpha, -0.05 * alpha, 0.07 * yaw_sign * alpha],
                lin_vel=[0.0, 0.0, 0.0],
                ang_vel=[0.0, 0.0, 0.0],
                stride_height=0.06,
                t_sw=0.2,
        ...
        if t < 4.0:
            alpha = smooth_step(t - 2.1, 0.7)
            return make_command(
                enabled=True,
                standing=False,
                pos=[0.0, 0.0, 0.2],
                orientation=[0.0, 0.0, 0.0],
                lin_vel=[0.28 * alpha, 0.0, 0.0],
                ang_vel=[0.0, 0.0, 0.0],
                stride_height=0.08,
                t_sw=0.21,
                t_st=0.3,
        ...
        if t < 5.5:
            alpha = smooth_step(t - 4.0, 0.5)
            return make_command(
                enabled=True,
                standing=False,
                pos=[0.0, 0.0, 0.2],
                orientation=[0.0, 0.0, 0.0],
                lin_vel=[0.18, 0.10 * lateral_sign * alpha, 0.0],
                ang_vel=[0.0, 0.0, 0.05 * yaw_sign * alpha],
                stride_height=0.085,
                t_sw=0.21,
                t_st=0.3,
            )
        if t < 7.0:
            alpha = smooth_step(t - 5.5, 0.5)
            return make_command(
                enabled=True,
                standing=False,
                pos=[0.0, 0.0, 0.2],
                orientation=[0.0, 0.0, 0.0],
                lin_vel=[0.18, 0.05 * lateral_sign, 0.0],
                ang_vel=[0.0, 0.0, 0.22 * yaw_sign * alpha],
                stride_height=0.085,
                t_sw=0.21,
                t_st=0.3,
            )
        return make_command(
            ...
            pos=[0.0, 0.0, 0.2],
            orientation=[0.0, 0.0, 0.0],
            lin_vel=[0.08, -0.06 * lateral_sign, 0.0],
            ang_vel=[0.0, 0.0, 0.12 * yaw_sign],
            stride_height=0.08,
            t_sw=0.21,
            t_st=0.3,
        )
...
def evaluate_candidate(
    ...
    try:
        ...
        while time.monotonic() - start < scenario.duration:
            ...
            command = scenario.schedule_fn(time.monotonic() - start)
...
def main() -> int:
    ...
    parser.add_argument("--short-duration", type=float, default=4.4)
    ...
    final_scenarios = [
        ValidationScenario("left_dynamic", build_validation_schedule(1.0, 1.0), args.final_duration),
        ValidationScenario("right_dynamic", build_validation_schedule(-1.0, -1.0), args.final_duration),
    ]
