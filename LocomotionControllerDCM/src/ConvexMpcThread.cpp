    ...
    while (running)
    {
        ...
        if (en_snapshot) {
            ...
            const VectorXd ref_grf_local =
                mpc.get_contact_forces(x0, x_ref_vec_snapshot, foot_positions, gait_table);
...
void ConvexMPCThread::set_observation_data(const RobotData& robot_state, const LegData& leg_state, const VectorXd& x_ref_vec,
