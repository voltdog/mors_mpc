...
int main() {
    ...
    if (!lcmExch.initialized) {
        cerr << "[LocomotionControllerDCM]: Failed to initialize LCM exchanger" << endl;
        return -1;
    }
    ...
    Vector3d ref_x_com_fb, ref_v_com_fb;
    ref_x_com_fb.setZero();
    ref_v_com_fb.setZero();
    Vector3d dcm_dd_x_com_ref;
    dcm_dd_x_com_ref.setZero();

    // swing controller
    VectorXd base_rpy_rate(3);
    VectorXd v(3);
    std::vector<std::array<double, 3>> p_finish(4);
    ...
    SwingController swing_controller(module_dt, 
                                    ...
                                    dz_near_ground);
    // foot step generator
    FootStepGenerator footstep_generator;
    footstep_generator.set_gait_params(t_sw, t_st);
    footstep_generator.set_heightmap(vision_map);
    footstep_generator.set_parameters(robot.bx, 
                                    robot.by, 
                                    robot.l1, 
                                    max_leg_length, 
                                    interleave_x, 
    ...
    cout << "[LocomotionControllerDCM]: Started" << endl;
    ...
    while(true)
    {
        ...
        if (enable == true)
        {
            ...
            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < 3; ++j) {
                    p_finish[i][j] = foot_plan.foot_sequence[1][i][j];
                    // p_finish_swing[i][j] = foot_plan.foot_sequence[1][i][j];
                }
            }
            ...
            ref_generator.set_body_adaptation_mode(adaptation_type);
            ref_generator.step(phase_signal, 
                               foot_pos_global,
                               foot_plan,
                               robot_cmd,
                               robot_state,
            ...
            robot_ref.lin_acc = dcm_dd_x_com_ref;
            {
            ...
            if (debug_mode)
            {
                ...
                lcmExch.sendFootstepSequence(foot_plan.foot_sequence, t);
                const auto dcm_com_trajectories = ref_generator.getDcmComTrajectories();
                lcmExch.sendDcmComTrajectory(dcm_com_trajectories.x_com,
                                             dcm_com_trajectories.d_x_com,
                                             dcm_com_trajectories.dd_x_com,
                                             t);
            }
