...
void ConvexMPC::calc_QP_matrices(const int planning_horizon_steps, const VectorXd& x0, const VectorXd& state_ref_vec,
    ...
    Sparse_Matrix& H_sparse, VectorXd& q_dense)
{
    ...
    const int expected_state_ref_size = state_dim * planning_horizon_steps;

    if (state_ref_vec.size() != expected_state_ref_size) {
        throw std::runtime_error("calc_QP_matrices expects state_ref_vec size to match the MPC horizon");
    }
...
VectorXd ConvexMPC::get_contact_forces(const VectorXd& x0, const VectorXd& x_ref_vec,
                                      const MatrixXd& foot_positions, const vector<int>& contact_state)
{
    ...
    calc_QP_matrices(horizon, x0, x_ref_vec, Ad, Bd, H, q);
