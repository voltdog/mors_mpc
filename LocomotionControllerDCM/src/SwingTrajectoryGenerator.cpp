...
SwingTrajectoryGenerator::SwingTrajectoryGenerator(double sim_freq)
    : sim_freq(sim_freq), inc(1.0 / sim_freq), dz_near_ground(0.0),
      ...
      p_start(4), p_rise(4), p_finish(4), d_p_start(4), dd_p_start(4),
      segment_duration(4, 0.0), single_segment_z(4, false),
...
std::tuple<double, double, double> SwingTrajectoryGenerator::map_z_rising(int leg_num, double it,
    ...
    const std::array<double, 3>& d_p_start, const std::array<double, 3>& dd_p_start,
    double tf, int /*cnt*/) {

    a_zr[leg_num] = calc_a(p_start[Z], p_rise[Z], tf + inc, d_p_start[Z], 0.0, dd_p_start[Z], 0.0);
...
std::tuple<double, double, double> SwingTrajectoryGenerator::map_xy_swing(double it,
    double p_start, double p_finish, double d_p_start, double dd_p_start, double tf) {

    auto a = calc_a(p_start, p_finish, tf, d_p_start, 0.0, dd_p_start, 0.0);
...
void SwingTrajectoryGenerator::set_parameters(double t_swing, double dz_near_ground) {
    ...
    std::fill(segment_duration.begin(), segment_duration.end(), tf);
}
...
void SwingTrajectoryGenerator::set_points(const std::vector<std::array<double, 3>>& start,
                                          ...
                                          const std::vector<std::array<double, 3>>& dd_start) {
    p_start = start;
    p_rise = rise;
    p_finish = finish;
    d_p_start = d_start;
...
void SwingTrajectoryGenerator::set_leg_timing(const std::vector<double>& duration,
                                              const std::vector<bool>& single_z) {
    for (int i = 0; i < 4; ++i) {
        if (i < static_cast<int>(duration.size()) && duration[i] > inc) {
            segment_duration[i] = duration[i];
        } else {
            segment_duration[i] = std::max(tf, inc);
        }

        single_segment_z[i] = i < static_cast<int>(single_z.size()) && single_z[i];
    }
}
...
SwingTrajectoryGenerator::step(const std::vector<double>& it, const std::vector<int>& cnt,
    ...
    for (int i = 0; i < 4; ++i) {
        if (leg_phase[i] == SWING) {
            const double tf_i = std::max(segment_duration[i], inc);
            const double t_i = std::clamp(it[i], 0.0, tf_i);
            auto [p_x_val, d_p_x_val, dd_p_x_val] = map_xy_swing(t_i, p_start[i][X], p_finish[i][X], d_p_start[i][X], dd_p_start[i][X], tf_i);
            auto [p_y_val, d_p_y_val, dd_p_y_val] = map_xy_swing(t_i, p_start[i][Y], p_finish[i][Y], d_p_start[i][Y], dd_p_start[i][Y], tf_i);

            double p_z_val, d_p_z_val, dd_p_z_val;
            if (single_segment_z[i]) {
                auto a_z = calc_a(p_start[i][Z], p_finish[i][Z], tf_i,
                                  d_p_start[i][Z], dz_near_ground,
                                  dd_p_start[i][Z], 0.0);
                std::tie(p_z_val, d_p_z_val, dd_p_z_val) = calc_spline(a_z, t_i);
            } else if (t_i < tf_i * rising_proportion) {
                std::tie(p_z_val, d_p_z_val, dd_p_z_val) = map_z_rising(i, t_i, p_start[i], p_rise[i], d_p_start[i],
                                                                        dd_p_start[i], tf_i * rising_proportion, cnt[i]);
            } else {
                std::tie(p_z_val, d_p_z_val, dd_p_z_val) = map_z_descending(i, (t_i - tf_i*rising_proportion),
                                                                            p_rise[i], p_finish[i],
                                                                            tf_i * (1-rising_proportion), cnt[i],
                                                                            dz_near_ground);
            }
