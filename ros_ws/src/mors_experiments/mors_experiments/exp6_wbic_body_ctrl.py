...
# MAX_REF_VEL_X_SLOW = 0.6
# MAX_REF_VEL_Y_SLOW = 0.6
# MAX_REF_VEL_Z_SLOW = 1.4
...
# # GALLOP
# MAX_REF_VEL_X_SLOW = 2.0
# MAX_REF_VEL_Y_SLOW = 0.4
# MAX_REF_VEL_Z_SLOW = 0.8
# STRIDE_HEIGHT_SLOW = 0.05
# T_SW_SLOW = 0.2
# T_ST_SLOW = 0.12
# GAIT_TYPE_SLOW = [0.0, 0.2, 0.5, 0.7]

# # PRONKING
# MAX_REF_VEL_X_SLOW = 2.0
# MAX_REF_VEL_Y_SLOW = 0.4
# MAX_REF_VEL_Z_SLOW = 0.8
# STRIDE_HEIGHT_SLOW = 0.08
# T_SW_SLOW = 0.2
# T_ST_SLOW = 0.12
# GAIT_TYPE_SLOW = [0.0, 0.0, 0.0, 0.0]
...
# T_SW_SLOW = 0.2
# T_ST_SLOW = 0.12
# GAIT_TYPE_SLOW = [0.0, 0.0, 0.5, 0.5]
...
# T_SW_SLOW = 0.2
# T_ST_SLOW = 0.3
# GAIT_TYPE_SLOW = [0.0, 0.5, 0.0, 0.5]

# TROT MAX TRAVERSABILITY
# THE BEST WITH Z=0.18
MAX_REF_VEL_X_SLOW = 0.3 # 0.5
MAX_REF_VEL_Y_SLOW = 0.0 # 0.4
MAX_REF_VEL_Z_SLOW = 0.8
STRIDE_HEIGHT_SLOW = 0.12 #0.13
T_SW_SLOW = 0.16 #0.21 #0.25
T_ST_SLOW = 0.16 #0.5 #0.17 #0.4
GAIT_TYPE_SLOW = [0.0, 0.5, 0.5, 0.0]

MAX_REF_VEL_X_MIDDLE = 1.2
MAX_REF_VEL_Y_MIDDLE = 0.4
MAX_REF_VEL_Z_MIDDLE = 2.0
STRIDE_HEIGHT_MIDDLE = 0.05
T_SW_MIDDLE = 0.16
T_ST_MIDDLE = 0.16
GAIT_TYPE_MIDDLE = [0.0, 0.5, 0.5, 0.0]

MAX_REF_VEL_X_FAST = 3.0
MAX_REF_VEL_Y_FAST = 0.4
MAX_REF_VEL_Z_FAST = 4.0
...
class RadiolinkTeleop(Node):
    def __init__(self):
        ...
        self.cmd_pose_msg.linear.z = 0.2
    ...
    def timer_callback(self):
        ...
        if self.R1_cur == 1 and self.pre_R1 == 0 and self.is_sleeping == True:
            # self.get_logger().info(f"Clicked! Go to Z: {cmd_pos_z1}")
        ...
        if self.is_sleeping == False:
            ...
            if self.robot_mode == LOCOMOTION_MODE:
                ...
                if self.gait_mode == SLOW_MODE:
                    if np.abs(self.joy_L3_x) < 0.04:
                        self.cmd_vel_msg.linear.x = 0.0    
                    else:
                        self.cmd_vel_msg.linear.x = self.joy_L3_x * MAX_REF_VEL_X_SLOW

                    if np.abs(self.joy_L3_y) < 0.04:
                        self.cmd_vel_msg.linear.y = 0.0
                    else:
                        self.cmd_vel_msg.linear.y = self.joy_L3_y * MAX_REF_VEL_Y_SLOW

                    if np.abs(self.joy_R3_y) < 0.04:
                        self.cmd_vel_msg.angular.z = 0.0
                    else:
                        self.cmd_vel_msg.angular.z = self.joy_R3_y * MAX_REF_VEL_Z_SLOW
                    ...
                    if np.abs(self.joy_L3_x) < 0.04:
                        ...
                        self.cmd_vel_msg.linear.x = self.joy_L3_x * MAX_REF_VEL_X_MIDDLE

                    if np.abs(self.joy_L3_y) < 0.04:
                        self.cmd_vel_msg.linear.y = 0.0
                    else:
                        self.cmd_vel_msg.linear.y = self.joy_L3_y * MAX_REF_VEL_Y_MIDDLE

                    if np.abs(self.joy_R3_y) < 0.04:
                        self.cmd_vel_msg.angular.z = 0.0
                    else:
                        self.cmd_vel_msg.angular.z = self.joy_R3_y * MAX_REF_VEL_Z_MIDDLE
                    ...
                    if np.abs(self.joy_L3_x) < 0.04:
                        ...
                        self.cmd_vel_msg.linear.x = self.joy_L3_x * MAX_REF_VEL_X_FAST

                    if np.abs(self.joy_L3_y) < 0.04:
                        self.cmd_vel_msg.linear.y = 0.0
                    else:
                        self.cmd_vel_msg.linear.y = self.joy_L3_y * MAX_REF_VEL_Y_FAST

                    if np.abs(self.joy_R3_y) < 0.04:
                        self.cmd_vel_msg.angular.z = 0.0
                    else:
                        self.cmd_vel_msg.angular.z = self.joy_R3_y * MAX_REF_VEL_Z_FAST
                ...
                # self.get_logger().info(f'locomotion mode')
                
            elif self.robot_mode == STANDING_MODE:
                self.cmd_vel_msg.linear.x = 0.0
                self.cmd_vel_msg.linear.y = 0.0
                ...
                self.cmd_pose_msg.angular.x = self.joy_L3_y * BODY_ROLL_MAX
                self.cmd_pose_msg.angular.y = self.joy_R3_x * BODY_PITCH_MAX
                self.cmd_pose_msg.angular.z = self.joy_R3_y * BODY_YAW_MAX
                self.gait_params_msg.standing = True
                # self.get_logger().info(f'body mode')
