...
T_SW1 = 0.21
T_ST1 = 0.45
GAIT_TYPE1 = [0.0, 0.44, 0.12, 0.56]

#WALK
T_SW2 = 0.21
T_ST2 = 0.9
GAIT_TYPE2 = [0.0, 0.25, 0.5, 0.75]

#TROT
T_SW3 = 0.21
T_ST3 = 0.3
GAIT_TYPE3 = [0.0, 0.5, 0.5, 0.0]
...
class RadiolinkTeleop(Node):
    def __init__(self):
        ...
        self.gait_params_msg.t_st = T_ST1
        self.gait_params_msg.t_sw = T_SW1
        self.gait_params_msg.gait_offsets = GAIT_TYPE1
        ...
        self.get_logger().info(f'Experiment 2: Gait Switching')
    ...
    def timer_callback(self):
        ...
        if self.is_sleeping == False and self.robot_mode == EXPERIMENT_MODE:
            if self.cur_time < 1.0:
                self.ref_vel_x = 0.0
                self.ref_vel_y = 0.0
            elif 1.0 < self.cur_time < 4.0:
                self.ref_vel_x = CMD_VEL_X
                self.gait_params_msg.t_st = T_ST1
                self.gait_params_msg.t_sw = T_SW1
                self.gait_params_msg.gait_offsets = GAIT_TYPE1
            elif 4.0 < self.cur_time < 8.0:
                self.gait_params_msg.t_st = T_ST2
                self.gait_params_msg.t_sw = T_SW2
                self.gait_params_msg.gait_offsets = GAIT_TYPE2
            elif 8.0 < self.cur_time < 10.0:
                self.gait_params_msg.t_st = T_ST3
                self.gait_params_msg.t_sw = T_SW3
                self.gait_params_msg.gait_offsets = GAIT_TYPE3
            elif 10.0 < self.cur_time < 11.0:
                self.ref_vel_x = 0.0
                self.ref_vel_y = 0.0
            else:
                self.robot_mode = STANDING_MODE
