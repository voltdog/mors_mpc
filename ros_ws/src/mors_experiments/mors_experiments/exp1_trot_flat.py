...
class RadiolinkTeleop(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Experiment 1: Trot on FLat Terrain')
    ...
    def timer_callback(self):
        ...
        if self.is_sleeping == False and self.robot_mode == EXPERIMENT_MODE:
            if self.cur_time < 1.0:
                ...
                self.ref_vel_x = CMD_VEL_X
                self.ref_vel_y = 0.0
            elif SWITCH_INTERVAL+1 < self.cur_time < 2*SWITCH_INTERVAL+1:
                self.ref_vel_x = -CMD_VEL_X
                self.ref_vel_y = -CMD_VEL_Y
            elif 2*SWITCH_INTERVAL+1 < self.cur_time < 3*SWITCH_INTERVAL+1:
                if CMD_VEL_Y == 0.0:
                    self.cur_time = 3*SWITCH_INTERVAL+1
                self.ref_vel_x = 0.0
                self.ref_vel_y = CMD_VEL_Y
            elif 3*SWITCH_INTERVAL+1 < self.cur_time < 3*SWITCH_INTERVAL+3:
                self.ref_vel_x = 0.0
                self.ref_vel_y = 0.0
            else:
                self.robot_mode = STANDING_MODE
