...
from additional.motor_simple import MotorSimple
from additional.motor_accurate import MotorAccurate
from additional.world_creator import WorldCreator
from additional.lidar import Lidar
...
# INIT_ORIENTATION = [0, -0.0998334, 0, 0.9950042 ]
...
class MorsBulletEnv(gym.Env):
  ...
  def __init__(self,
    ...
    Args:
      ...
      step_enabled: If True, it is needed to send motor commands as arguments in 
        step() function. If False, it is needed to use set_motor_commands() function. 
    ...
    self.force = 136
    self.kin_scheme = init_kin_scheme

    self._lateral_friction = 1.5
    self._spinning_friction = 1.63
    ...
    self._init_orientation = init_orient
    ...
    self._pybullet_client.resetDebugVisualizerCamera(0.8, 40.0, -40.0, [0,0,0])
  ...
  def step(self, action):
    ...
    """
    # if self._on_rack:
    #     self.base_pitch -= 0.7*1/500
    #     print(self.base_pitch)
    #     rot = Rotation.from_euler('xyz', [0, self.base_pitch, 0], degrees=False).as_quat()
    #     self._pybullet_client.createConstraint(self.quadruped, BASE_LINK_ID, -1, -1,
    #                                            self._pybullet_client.JOINT_FIXED, [0, 0, 0],
    #                                            [0, 0, 0], self._init_position, [0, 0, 0, 1], rot)
    # debug parameters
  ...
  def _ResetPoseForLeg(self, leg_id):
    ...
    print("===============================")
    print(self.kin_scheme)
    print("===============================")
    if self.kin_scheme == "m":
      angle = [0.25, -0.8, 1.64, 
              -0.25, 0.8, -1.64,
              -0.25, -0.8, 1.64,
              0.25, 0.8, -1.64]
    elif self.kin_scheme == "x":
      angle = [0.25, -0.8, 1.64, 
               -0.25, 0.8, -1.64,
               -0.25, 0.8, -1.64,
               0.25, -0.8, 1.64]
    elif self.kin_scheme == "o":
      angle = [-0.25, 0.8, -1.64, 
               0.25, -0.8, 1.64,
              -0.25, -0.8, 1.64,
              0.25, 0.8, -1.64]
    elif self.kin_scheme == ">":
      angle = [-0.25,  0.8, -1.64, 
               0.25, -0.8, 1.64,
               -0.25, 0.8, -1.64,
               0.25, -0.8, 1.64]
    elif self.kin_scheme == "mix":
      angle = [-0.25,  -0.8, 1.64, 
               0.25, -0.8, 1.64,
               -0.25, 0.8, -1.64,
               0.25,  0.8, -1.64]
    ...
    self._pybullet_client.resetJointState(self.quadruped,
                                          ...
                                          self._motor_direction[2 * leg_id] * angle[3*leg_id],
                                          targetVelocity=0)
    self._pybullet_client.resetJointState(self.quadruped,
                                          self._joint_name_to_id["hip_joint_" + leg_position],
                                          self._motor_direction[2 * leg_id] * angle[3*leg_id+1],
                                          targetVelocity=0)
    self._pybullet_client.resetJointState(self.quadruped,
                                          self._joint_name_to_id["knee_joint_" + leg_position],
                                          self._motor_direction[2 * leg_id + 1] * angle[3*leg_id+2],
                                          targetVelocity=0)
  ...
  def _reset_robot(self, reload_urdf=True):
    ...
    if reload_urdf:
      if self._self_collision_enabled:
        ...
        self.quadruped = self._pybullet_client.loadURDF(
            addr, self._init_position)
      if self._on_rack:
        self._pybullet_client.createConstraint(self.quadruped, BASE_LINK_ID, -1, -1,
                                               self._pybullet_client.JOINT_FIXED, [0, 0, 0],
                                               [0, 0, 0], self._init_position, [0, 0, 0, 1], self._init_orientation)
      ...
      self._pybullet_client.resetBasePositionAndOrientation(self.quadruped, self._init_position,
  ...
  def get_base_ang_vel(self):
    (vx, vy, vz), (vr, vp, vy) = self._pybullet_client.getBaseVelocity(self.quadruped)
    # rotate to robot frame
    _, (x, y, z, w) = self.get_base_position_and_orientation()
    # rotation matrix
    M = quat2mat((w, x, y, z))
    # velocities as vector
    v = np.array([vr, vp, vy])
    angular_vel_robot_frame = np.matmul(M, v)
  ...
  def get_contact_flags(self):
    ...
    for contact_point in self.contact_points:
      if contact_point[3] == 4:
          contact_flags[0] = True
      if contact_point[3] == 8:
          contact_flags[1] = True
      if contact_point[3] == 12:
          contact_flags[2] = True
      if contact_point[3] == 16:
