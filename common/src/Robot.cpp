    ...
    double damping)
{
    ...
    for (int iter = 0; iter < max_iter; ++iter)
    {
        ...
        Eigen::VectorXd dq_legs =
            gain * J.transpose() * H.ldlt().solve(error);

        // --- Формируем полный dq ---
        Eigen::VectorXd dq = Eigen::VectorXd::Zero(model_.nv);

        for (int i = 0; i < 4; ++i)
            dq.segment(jac_pos_indicies[i], 3) =
                dq_legs.segment<3>(3*i);
    ...
    std::vector<Eigen::Vector3d>& h_legs)
{
    ...
    pinocchio::computeAllTerms(model_, data_, q, q_dot);
    pinocchio::computeJointJacobians(model_, data_, q);

    Eigen::LDLT<Eigen::MatrixXd> ldlt_M(data_.M);

    Eigen::VectorXd Minv_h =
        ldlt_M.solve(data_.nle);

    for(int i = 0; i < 4; ++i)
    {
        pinocchio::FrameIndex frame_id =
            model_.getFrameId(ef_frames[i]);
        ...
        Eigen::MatrixXd J =
            J_full.block(0, 0, 3, model_.nv);

        Eigen::MatrixXd Minv_JT =
            ldlt_M.solve(J.transpose());

        Eigen::Matrix3d Lambda_inv =
            J * Minv_JT;

        M_legs[i] = Lambda_inv.inverse();
