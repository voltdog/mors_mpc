...
std::vector<Eigen::Matrix3d> Robot::GetFootJacobianWorld(const VectorXd &q)
{
    std::vector<Eigen::Matrix3d> jacobians;
    int i = 0;
    ...
    for (const auto& frame_name : ef_frames)
    {
        ...
        pinocchio::getFrameJacobian(model_, *data_, frame_id, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED, J_full);
        jacobians.push_back(J_full.block(0, jac_pos_indicies[i], 3, 3));
...
std::vector<Eigen::Matrix3d> Robot::GetFootJacobianDotWorld(const VectorXd &q, const VectorXd &v)
{
    std::vector<Eigen::Matrix3d> jacobian_dots;
    int i = 0;
    ...
    {
        pinocchio::FrameIndex frame_id = model_.getFrameId(frame_name);
        Eigen::MatrixXd dJ_full(6, model_.nv);
        dJ_full.setZero();

        pinocchio::getFrameJacobianTimeVariation(model_, *data_, frame_id, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED, dJ_full);
        jacobian_dots.push_back(dJ_full.block(0, jac_pos_indicies[i], 3, 3));

        i++;
    }
    return jacobian_dots;
}

void Robot::GetLegDynamics(const Eigen::VectorXd& q,
                           const Eigen::VectorXd& v,
                           std::vector<Eigen::Matrix3d>& M_legs,
                           std::vector<Eigen::Vector3d>& C_legs,
                           std::vector<Eigen::Vector3d>& G_legs)
{
    pinocchio::crba(model_, *data_, q);                     // M(q) -> data_->M
    data_->M.triangularView<Eigen::StrictlyLower>() =
        data_->M.transpose().triangularView<Eigen::StrictlyLower>();
    pinocchio::nonLinearEffects(model_, *data_, q, v);      // C(q,v)*v + g(q) -> data_->nle
    pinocchio::computeGeneralizedGravity(model_, *data_, q); // g(q) -> data_->g

    const int start_idx = 6; // nv-индекс первого сустава ноги (после 6 dof базы)
    for (int leg = 0; leg < 4; leg++)
    {
        int idx = start_idx + leg * 3;

        M_legs[leg] = data_->M.block(idx, idx, 3, 3);
        G_legs[leg] = data_->g.segment(idx, 3);
        C_legs[leg] = data_->nle.segment(idx, 3) - G_legs[leg]; // C(q,v)*v = nle - g
    }
}
    ...
    std::vector<Eigen::Vector3d>& h_legs)
{
    ...
    int start_idx = 6; // nv-индекс первого сустава ноги (после 6 dof базы)
